...
private:
    ...
    void updateText();
    void loadMovies();
    void addMoviesFromVector();
    void checkForAndDeleteMovie(string name);
    void saveMovies();
    void setDefaultSummaryText();
    void prepareToLoadMovie();

    string checkForLoadingErrors(vector<string> tokens, int number);

    Movie::Rating determineRating(string ratingEntered);
